...
CREATE DEFINER=`prof1234`@`localhost` PROCEDURE `recupReservationByNom` (IN `paramNom` VARCHAR(255))   BEGIN
	SELECT * FROM Reservation
	WHERE nom = paramNom;
END$$
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `updateReservation` (IN `paramNom` VARCHAR(255), IN `paramCoordonne` VARCHAR(255), IN `paramNbrPerson` INT, IN `paramAdulte` INT, IN `paramEnfant` INT, IN `paramNumChambre` VARCHAR(255), IN `paramActuChambre` VARCHAR(255))   BEGIN
	UPDATE Reservation 
...
(8, 'Jean', 'Lasale', 'j.lasale@labiz.com', 600000000, '2022-04-12', '2022-04-17', 'D105');
